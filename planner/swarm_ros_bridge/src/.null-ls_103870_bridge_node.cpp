    ...
    if (recv_flag = receivers[i]->receive(recv_array, dont_block)) {
      ...
      */
      ...
      memcpy(recv_buffer.get(),
             static_cast<const uint8_t *>(recv_array.raw_data(recv_array.read_cursor())), data_len);
...
int main(int argc, char **argv) {
  ...
  if (len_send > SUB_MAX) {
    ROS_FATAL("[bridge_node] The number of send topics in configuration exceeds the limit %d!",
              SUB_MAX);
    return 2;
  }
