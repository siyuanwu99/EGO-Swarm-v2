...
void EGOReplanFSM::init(ros::NodeHandle &nh) {
  ...
  mandatory_stop_sub_ =
      nh.subscribe("mandatory_stop", 1, &EGOReplanFSM::mandatoryStopCallback, this);
  ...
  broadcast_ploytraj_sub_ = nh.subscribe<traj_utils::MINCOTraj>(
      "planning/broadcast_traj_recv", 100, &EGOReplanFSM::RecvBroadcastMINCOTrajCallback, this,
  ...
  switch (exec_state_) {
    ...
    case EXEC_TRAJ: {
      ...
      if (mondifyInCollisionFinalGoal())  // case 1: find that current goal is in obstacles
      {
        // pass
                 ...
                 (final_goal_ - pos).norm() <
                     no_replan_thresh_)  // case 2: assign the next waypoint
      {
        wpt_id_++;
        planNextWaypoint(wps_[wpt_id_]);
      } else if ((t_cur > info->duration - 1e-2) &&
                 touch_the_goal)  // case 3: the final waypoint reached
      {
        have_target_  = false;
        have_trigger_ = false;
        ...
        changeFSMExecState(WAIT_TARGET, "FSM");
                 ...
                 (!touch_the_goal &&
                  close_to_current_traj_end))  // case 3: time to perform next replan
      {
        changeFSMExecState(REPLAN_TRAJ, "FSM");
      }
      // ROS_ERROR("AAAA");
    ...
    case EMERGENCY_STOP: {
      if (flag_escape_emergency_)  // Avoiding repeated calls
      {
        ...
        if (enable_fail_safe_ && odom_vel_.norm() < 0.1) changeFSMExecState(GEN_NEW_TRAJ, "FSM");
      }
...
bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj) {
  planner_manager_->getLocalTarget(planning_horizen_, start_pt_, final_goal_, local_target_pt_,
                                   local_target_vel_, touch_goal_);

  bool plan_success = planner_manager_->reboundReplan(
      start_pt_, start_vel_, start_acc_, local_target_pt_, local_target_vel_,
      (have_new_target_ || flag_use_poly_init), flag_randomPolyTraj, touch_goal_);
...
bool EGOReplanFSM::planFromLocalTraj(const int trial_times /*=1*/) {
  ...
  ROS_INFO("[PROJ] Replan time: %f ms", (tf - t0).toSec() * 1000);
  ...
  if (planner_manager_->grid_map_->getInflateOccupancy(final_goal_)) {
    ...
    for (double t = planner_manager_->traj_.global_traj.duration; t > 0; t -= t_step) {
      ...
      if (!planner_manager_->grid_map_->getInflateOccupancy(pt)) {
        ...
        {
          ROS_INFO(
              "Current in-collision waypoint (%.3f, %.3f %.3f) has been modified to (%.3f, %.3f "
              "%.3f)",
...
void EGOReplanFSM::RecvBroadcastMINCOTrajCallback(const traj_utils::MINCOTrajConstPtr &msg) {
  ...
  if (planner_manager_->traj_.swarm_traj.size() > recv_id &&
    ...
    ROS_WARN("Received drone %d's trajectory out of order or duplicated, abandon it.",
             (int)recv_id);
    return;
  }
  ...
  if (abs((t_now - msg->start_time).toSec()) > 0.25) {
        ...
        10.0)  // 10 seconds offset, more likely to be caused by unsynced system time.
    {
      ROS_WARN("Time stamp diff: Local - Remote Agent %d = %fs", msg->drone_id,
               (t_now - msg->start_time).toSec());
    } else {
      ROS_ERROR(
          "Time stamp diff: Local - Remote Agent %d = %fs, swarm time seems not synchronized, "
          "abandon!",
  ...
  if (msg->start_time.toSec() <= planner_manager_->traj_.swarm_traj[recv_id]
                                     .start_time)  // This must be called after buffer fill-up
  {
  ...
  headState << msg->start_p[0], msg->start_v[0], msg->start_a[0], msg->start_p[1], msg->start_v[1],
      msg->start_a[1], msg->start_p[2], msg->start_v[2], msg->start_a[2];
  tailState << msg->end_p[0], msg->end_v[0], msg->end_a[0], msg->end_p[1], msg->end_v[1],
      msg->end_a[1], msg->end_p[2], msg->end_v[2], msg->end_a[2];
  ...
  for (int i = 0; i < piece_nums; i++) durations(i) = msg->duration[i];
...
void EGOReplanFSM::polyTraj2ROSMsg(traj_utils::PolyTraj  &poly_msg,
  ...
  for (int i = 0; i < piece_num; i++) MINCO_msg.duration[i] = durs[i];
}
